...
const BccDistribution = () => {
  ...
  const handelSearchDate = ()=>{
    ...
    const getMyCampDetailsByEmpcode = async()=>{
     setLoading(true)
      ...
      try {
        const res = await axios.post(`${BASEURL2}/admin/getBccCampDetailsByEmpcode`,{
          empcode:empcode,
          searchKeyword:"",
          campType:""
        })
    ...
    async function GetDetiledData(){
      ...
      try {
          const res = await axios.post(`${BASEURL2}/admin/getBccSheetReport`,params);
  ...
  const handelReportDownloadDetailed = () => {
    const headers = [
      ...
      'Doctor Name', 'Doctor Garnet Code', 'Doctor Speciality', 'BCC Distributed', 'Prescription Date',
    ...
    const mappedData = allReportData.map(item => ({
      ...
      'BCC Distributed': item.BCC_Distributed === "Y" ? "Yes" : "No" || item.BCC_Distributed === "Y" ? "Yes" : "No" || '',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'BccDistributionReport');
    XLSX.writeFile(wb, 'BccDistributionReport.xlsx');
  };
...
export default BccDistribution;