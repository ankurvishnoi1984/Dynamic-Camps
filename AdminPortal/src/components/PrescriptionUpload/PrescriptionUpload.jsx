...
const PrescriptionUpload = () => {
  ...
  const handelSearchDate = ()=>{
    ...
    const getMyCampDetailsByEmpcode = async()=>{
     setLoading(true)
      ...
      try {
        const res = await axios.post(`${BASEURL2}/admin/getPrescriptionUploadByEmpcode`,{
          empcode:empcode,
          searchKeyword:"",
          campType:""
        })
    ...
    console.log("campDetails : ",myCampDetails)
    ...
    async function GetDetiledData(){
      ...
      try {
          const res = await axios.post(`${BASEURL2}/admin/getPrescriptionSheetReport`,params);
  ...
  const handelReportDownloadDetailed = () => {
    const headers = [
      'Rank', 'Employee Name', 'Area', 'Region', 'Zone', 'HQ',
      'Doctor Name', 'Doctor Garnet Code', 'Doctor Speciality', 'Empanorm Launched', 'Prescription Date',
      'TSARTTrio', 'TSARTAM', 'TSARTMCL', 'TSART', 'Jupiros', 'JupirosGold', 'JupirosF', 'JupirosCV', 'Ticavic', 'Apixapil', 'JupiorosCv20', 'Bisokem', 'Bisokem AM', 'Empanorm', 'Empanorm Duo', 'Empanorm L', 'Empanorm M', 'Jupiros EZ', 'Empanorm 25 mg', 'Empanorm Trio', 'Others'
    ];
    ...
    const mappedData = allReportData.map(item => ({
      ...
      'Empanorm Launched': item.Empanorm_Launched === "Y"?"Yes":"No" || item.is_empanorm_launched=== "Y"?"Yes":"No" || '',
      'Prescription Date': item.Prescription_Date || item.date_of_prescription || '',
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'PrescriptionUploads');
    XLSX.writeFile(wb, 'PrescriptionUploads.xlsx');
  };
                    ...
                    <tbody>
                        ...
                        .map((e)=>{
                           ...
                           <tr key={e.empcode}>
                            ...
                            <td>{e.speciality}</td>
                            <td>{e.garnet_code}</td>
                            <td>{e.is_empanorm_launched === "N"?"No":"Yes"}</td>
