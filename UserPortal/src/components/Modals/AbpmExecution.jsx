...
const AbpmExecution = ({ crid, show, handelCloseEditModal, campReportList }) => {
  ...
  const handleSubmit = async () => {
    if (!screenedCount || !isPrescriptionGen) {
      setPopup({
        type: "error",
    ...
    try {
      ...
      if (res2.data.errorCode !== 1) {
        setPopup({ type: "error", message: res1.data.message || "Error saving brand images" });
        return;
      }

      // handelCloseEditModal();
      setPopup({ type: "success", message: "Camp executed & brand images saved successfully!" });
    } catch (error) {
      console.error(error);
