...
const DiabetesModal = ({ crid, show, handelCloseEditModal, campReportList }) => {
  const campData = campReportList.find(
    (el) => Number(el.crid) === Number(crid)
  );

  const [screenedCount, setScreenedCount] = useState(0);
  const [glucometerKitCount,setGlucometerKitCount] =useState(0);
...
const handleSubmit = async () => {
  if (!userId || !screenedCount || !isPrescriptionGen) {
    setPopup({
      type: "error",
  ...
  try {
    ...
    // 2️⃣ Execute camp
    const res1 = await axios.post(`${BASEURL2}/camp/executeDiabetesCamp`, {
      campId: crid,
      userId: userId,
      patientScreened: screenedCount,
      isPrescriptionGen: isPrescriptionGen,
      glucometerKitCount:glucometerKitCount,
    });
    ...
    // 3️⃣ Build FormData for brand images
                ...
                <div className="form-group col-md-4 did-floating-label-content">
                 ...
                 <div className="form-group col-md-4 did-floating-label-content">
                  <input
                    type="number"
                    className="form-control did-floating-input"
                    onChange={(e) => setGlucometerKitCount(e.target.value)}
                    placeholder="Glucometer Kits Given"
                    value={glucometerKitCount}
                    onKeyDown={(e) => {
                      if (e.key === "-" || e.key === "e") e.preventDefault();
                    }}
                    min={0}
