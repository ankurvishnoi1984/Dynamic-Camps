...
import AddEmpanormModal from "../Modals/AddEmpanormModal";
import "./EmpCss.css";
...
const handleClosePreview = () => {
  ...
  const getCampReportImages = async (crid) => {
    try {
      ...
      if (res?.status === 200) {
        console.log("getImages",res?.data?.data);
  ...
  const getCampReportList = async () => {
    ...
    try {
      const res = await axios.post(
        ...
        { ...filters, userId,activityId:'1' }
      );
        // console.log("res for camp report", res)
      if (res?.data?.errorCode == "1") {
        console.log("getCampReportListWithImage",res?.data?.data);
    ...
    const handelReportDownload = () => {
      ...
      XLSX.writeFile(wb, `Empanorm_Blitz_Reports_${rn}.xlsx`);
  ...
  const handleAddSubmit = async () => {
    ...
    formData.append("noOfKits",noOfKits)
    let res;
...
const handelUpdateBrandMapping = async(reportId)=>{
  ...
  const handleBrandChange = (selectedOptions) => {
    console.log("brand option ", selectedOptions)
    setSelectedBrands(selectedOptions);
  };
...
const groupedImages = infoData.images?.reduce((acc, img) => {
      ...
      if (lastItem.mappingId) {
        try {
          ...
          let res = await axios.post(`${BASEURL}/report/deleteBrandMapping`,{mappingId:lastItem.mappingId});
            console.log("delete brand mapping",res)
                    ...
                    <table className="table table-hover newcss">
                      <thead>
                        <tr>
                          ...
                          <th scope="col">No of kits given</th>
        ...
        <div className="modal-content shadow-lg border-0 rounded-3">
          <div className="modal-header bg-primary text-white">
            <h5 className="modal-title fw-bold">
              <i className="bi bi-file-earmark-text me-2"></i> Camp Report Info
            </h5>
            <button
              type="button"
          ...
          <div className="modal-body p-4">
            ...
            <form className="row g-4">
              <div className="col-md-4">
                <label className="form-label fw-semibold">Type of Camp</label>
                <input
                  type="text"
                  className="form-control"
                  value={infoData?.camp_type_name || ""}
                  readOnly
                />
              </div>
              ...
              <div className="col-md-4">
                <label className="form-label fw-semibold">Date of Camp</label>
                <input
                  type="text"
                  className="form-control"
                  value={infoData?.camp_date1 || ""}
                  readOnly
                />
              </div>
            </form>
  ...
  <div className="image-preview-overlay" onClick={handleClosePreview}>
    ...
    <img src={previewImg} alt="Preview" className="preview-img" />
     ...
     {addRequestModel&& <AddEmpanormModal
